...
func (u *usecase) GetAllByAccountID(ctx context.Context, accountID types.ExternalID) ([]transfer.Transfer, error) {
	...
	u.logger.LogDebug(operation, "searching if the account exist")
	...
	u.logger.LogDebug(operation, "searching all the transfers that the accounts is involved")
	...
	u.logger.LogDebug(operation, "listing all the data successfully")
